...
void OPNMonitor::Mix(int32_t* s, int length) {
  int c = buf[write][0];
...
void OPNMonitor::UpdateText() {
  ...
  wsprintf(buf, "%.3x --/%.3x --/%.3x --/%.2x %.4x %x  ", (regs[0] + regs[1] * 256) & 0xfff,
           (regs[2] + regs[3] * 256) & 0xfff, (regs[4] + regs[5] * 256) & 0xfff, regs[6],
  ...
  wsprintf(buf + 32, "%.2x %.2x %.4x %.4x %.4x %.2x %.4x %.2x", regs[0x100], regs[0x101],
           (regs[0x102] + regs[0x103] * 256) & 0xffff, (regs[0x104] + regs[0x105] * 256) & 0xffff,
  ...
  //  -- - --- -- -- -- --
  {
    ...
    for (i = 8; i < 14; i++)
      ToHex(&ptr, regs[0x10 + i]), *ptr++ = ' ';
    Putf("%.2x %x %.3x %.2x %.2x %.2x %.2x             %.26s\n", regs[0x21], regs[0x22] & 0x0f,
         (regs[0x24] + regs[0x25] * 256) & 0x3ff, regs[0x26], regs[0x27], regs[0x28], regs[0x29],
         buf);
  }
  for (y = 3; y < 10; y++) {
    char* ptr = buf;
  ...
  if (regs[0x27] & 0xc0) {
    Putf("a: %.5x    %.5x    %.5x %.5x a: %.5x    %.5x    %.5x\n",
         ToFnum(regs[0x0a4] * 0x100 + regs[0x0a0]), ToFnum(regs[0x0a5] * 0x100 + regs[0x0a1]),
         ToFnum(regs[0x0ad] * 0x100 + regs[0x0a9]), ToFnum(regs[0x0ae] * 0x100 + regs[0x0aa]),
         ToFnum(regs[0x1a4] * 0x100 + regs[0x1a0]), ToFnum(regs[0x1a5] * 0x100 + regs[0x1a1]),
         ToFnum(regs[0x1a6] * 0x100 + regs[0x1a2]));
    Putf("b: %.4x  %.4x  %.4x  %.5x %.5x b: %.4x     %.4x     %.4x\n",
         ToFB(regs[0x0b4] * 0x100 + regs[0x0b0]), ToFB(regs[0x0b5] * 0x100 + regs[0x0b1]),
         ToFB(regs[0x0b6] * 0x100 + regs[0x0b2]), ToFnum(regs[0x0ac] * 0x100 + regs[0x0a8]),
         ToFnum(regs[0x0a6] * 0x100 + regs[0x0a2]), ToFB(regs[0x1b4] * 0x100 + regs[0x1b0]),
         ToFB(regs[0x1b5] * 0x100 + regs[0x1b1]), ToFB(regs[0x1b6] * 0x100 + regs[0x1b2]));
  } else {
    Putf("a: %.5x    %.5x    %.5x       a: %.5x    %.5x    %.5x\n",
         ToFnum(regs[0x0a4] * 0x100 + regs[0x0a0]), ToFnum(regs[0x0a5] * 0x100 + regs[0x0a1]),
         ToFnum(regs[0x0a6] * 0x100 + regs[0x0a2]), ToFnum(regs[0x1a4] * 0x100 + regs[0x1a0]),
         ToFnum(regs[0x1a5] * 0x100 + regs[0x1a1]), ToFnum(regs[0x1a6] * 0x100 + regs[0x1a2]));
    Putf("b: %.4x     %.4x     %.4x        b: %.4x     %.4x     %.4x\n",
         ToFB(regs[0x0b4] * 0x100 + regs[0x0b0]), ToFB(regs[0x0b5] * 0x100 + regs[0x0b1]),
         ToFB(regs[0x0b6] * 0x100 + regs[0x0b2]), ToFB(regs[0x1b4] * 0x100 + regs[0x1b0]),
         ToFB(regs[0x1b5] * 0x100 + regs[0x1b1]), ToFB(regs[0x1b6] * 0x100 + regs[0x1b2]));
  }
#endif
}
