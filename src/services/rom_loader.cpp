...
namespace {
constexpr int idx(pc8801::RomType t) {
...
void RomLoader::LoadPC88() {
  ...
  roms_[idx(pc8801::RomType::kN88Rom)] = std::make_unique<RomView>(0x8000);
  memcpy(roms_[idx(pc8801::RomType::kN88Rom)]->Get(), n88rom.get(), 0x8000);

  roms_[idx(pc8801::RomType::kNRom)] = std::make_unique<RomView>(0x8000);
  memcpy(roms_[idx(pc8801::RomType::kNRom)]->Get() + 0x6000, n88rom.get() + 0x8000, 0x2000);
  memcpy(roms_[idx(pc8801::RomType::kNRom)]->Get(), n88rom.get() + 0x16000, 0x6000);

  roms_[idx(pc8801::RomType::kN88ERom0)] = std::make_unique<RomView>(0x2000);
  memcpy(roms_[idx(pc8801::RomType::kN88ERom0)]->Get(), n88rom.get() + 0xc000, 0x2000);

  roms_[idx(pc8801::RomType::kN88ERom1)] = std::make_unique<RomView>(0x2000);
  memcpy(roms_[idx(pc8801::RomType::kN88ERom1)]->Get(), n88rom.get() + 0xe000, 0x2000);

  roms_[idx(pc8801::RomType::kN88ERom2)] = std::make_unique<RomView>(0x2000);
  memcpy(roms_[idx(pc8801::RomType::kN88ERom2)]->Get(), n88rom.get() + 0x10000, 0x2000);

  roms_[idx(pc8801::RomType::kN88ERom3)] = std::make_unique<RomView>(0x2000);
  memcpy(roms_[idx(pc8801::RomType::kN88ERom3)]->Get(), n88rom.get() + 0x12000, 0x2000);

  LoadRom("DISK.ROM", pc8801::RomType::kSubSystemRom, 0x2000);
  if (!IsAvailable(pc8801::RomType::kSubSystemRom)) {
    roms_[idx(pc8801::RomType::kSubSystemRom)] = std::make_unique<RomView>(0x2000);
    memcpy(roms_[idx(pc8801::RomType::kSubSystemRom)]->Get(), n88rom.get() + 0x14000, 0x2000);
  }
}
...
void RomLoader::LoadKanji() {
  ...
  if (!IsAvailable(pc8801::RomType::kFontRom)) {
    roms_[idx(pc8801::RomType::kFontRom)] =
        std::make_unique<RomView>(roms_[idx(pc8801::RomType::kKanji1Rom)]->Get() + 0x1000, 0x800);
  }
...
void RomLoader::LoadOptionalRoms() {
  ...
  for (int i = 1; i < 9; i++) {
    name[1] = '0' + i;
    auto type = static_cast<pc8801::RomType>(idx(pc8801::RomType::kExtRom1) + i - 1);
