...
int Z80Test::ExecDual(Z80Test* first, Z80Test* second, int nclocks) {
  ...
  first->readcount = first->writecount = first->readcountt = first->writecountt = first->codesize =
      0;
  first->cpu1.TestIntr();
  first->cpu2.TestIntr();
  second->readcount = second->writecount = second->readcountt = second->writecountt =
      second->codesize = 0;
...
void Z80Test::Test() {
  ...
  // レジスタの一致確認
  if (((reg1.r.b.a ^ reg2.r.b.a) | (reg1.r.w.bc ^ reg2.r.w.bc) | (reg1.r.w.de ^ reg2.r.w.de) |
       (reg1.r.w.hl ^ reg2.r.w.hl) | (reg1.r.w.ix ^ reg2.r.w.ix) | (reg1.r.w.iy ^ reg2.r.w.iy) |
...
void Z80Test::Error(const char* errtxt) {
  if (fp) {
    ...
    fprintf(fp, "%s  reads:ref=%d target=%d writes:ref=%d target=%d\n", errtxt, readcount,
            readcountt, writecount, writecountt);
    ...
    fprintf(fp,
            "     O PC:%.4x SP:%.4x AF:%.4x HL:%.4x DE:%.4x BC:%.4x IX:%.4x IY:%.4x IFF%d IRQ:%d "
            "R:%.2x\n",
            pc, reg.r.w.sp & 0xffff, reg.r.w.af & 0xffd7, reg.r.w.hl & 0xffff, reg.r.w.de & 0xffff,
            reg.r.w.bc & 0xffff, reg.r.w.ix & 0xffff, reg.r.w.iy & 0xffff, reg.iff1, intr,
            reg.rreg);
    ...
    fprintf(fp,
            ...
            reg1.r.w.de & 0xffff, reg1.r.w.bc & 0xffff, reg1.r.w.ix & 0xffff, reg1.r.w.iy & 0xffff,
            reg1.iff1, cpu1.IsIntr(), reg1.intmode, reg1.rreg);

    fprintf(fp,
            "     O PC:%.4x SP:%.4x AF:%.4x HL:%.4x DE:%.4x BC:%.4x IX:%.4x IY:%.4x IFF%d IRQ:%d "
            "IM:%d R:%.2x\n",
            cpu2.GetPC(), reg2.r.w.sp & 0xffff, reg2.r.w.af & 0xffd7, reg2.r.w.hl & 0xffff,
            reg2.r.w.de & 0xffff, reg2.r.w.bc & 0xffff, reg2.r.w.ix & 0xffff, reg2.r.w.iy & 0xffff,
            reg2.iff1, cpu2.IsIntr(), reg2.intmode, reg2.rreg);
  }
}
...
uint32_t Z80Test::S_Read8R(void* inst, uint32_t a) {
  return ((Z80Test*)(inst))->Read8R(a);
}

uint32_t Z80Test::S_Read8T(void* inst, uint32_t a) {
  return ((Z80Test*)(inst))->Read8T(a);
}

void Z80Test::S_Write8R(void* inst, uint32_t a, uint32_t d) {
  ((Z80Test*)(inst))->Write8R(a, d);
}

void Z80Test::S_Write8T(void* inst, uint32_t a, uint32_t d) {
